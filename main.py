...
class Truck:
    ...
    def deliver_package(self, packageId):
        deliveredPackage = self.removePackage(packageId)
        package_hash_table[deliveredPackage.packId].status = PackageStatus.delivered
        package_hash_table[deliveredPackage.packId].delivered_at = clock.time
    ...
    def update_delivery_times(self):
        ...
        for i in range(len(self.route)-1):
            self.times.append(calc_route_time([self.route[i], self.route[i+1]]) + self.times[i])
...
class Package:
    ...
    def __init__(self, packId, address, city, zipCode, deadline=None, weight=0, locationIndex=0, status=2):
        ...
        package_hash_table[packId]=self
    ...
    def __str__(self):
        return (
            "Package Id #" + str(self.packId) + " \t " + "Address: " + str(self.address) + "\t" + "Deadline: " + str(self.deadline) + " \t " + "City: " + str(
